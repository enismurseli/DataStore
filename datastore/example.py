...
# ftp=destination.Ftp(dirpath=Path.cwd() / "input_ouput_data",
...
local=destination.Local(dirpath=Path.cwd() / "input_ouput_data", filename="test_primitive_data.txt", seperator=";")
local.dump_data(ds_data, mode="wb", format='bytes')
ldata=local.load_data(RecordInstance=PrimitiveRecord, format='bytes')
ldata.print(5)

local=destination.Local(dirpath=Path.cwd() / "input_ouput_data", filename="test_json_data.json", seperator=";")
