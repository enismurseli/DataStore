...
class Local(AbstractDestination):
    ...
    def dump_data(self, new_data: Data, mode: str, format: ['json', 'bytes', 'xml']) -> bool:
        """
        ...
        if create_connection := self.create_connection(mode=mode):
            if format == 'bytes':
                ...
                jdata = new_data.data_as_dict()
                json.dump(jdata, self.connection)
            else:
                raise ValueError("This format of saving data is not supported!")
            return self.close_connection
        else:
            raise ValueError(f"Error could not open filepath {self.save_filepath}")

    def load_data(self, RecordInstance: AbstractRecord, format: ['json', 'bytes', 'xml']) -> Data:
        """
...
class Ftp(Local):
    ...
    def dump_data(self, new_data: Data, format: ['json', 'bytes', 'xml']) -> bool:
...
class S3(Local):
    def __init__(self, dirpath: Path, filename: str, region_name: str, aws_access_key_id: str,
                 aws_secret_access_key: str, bucket_name: str, seperator=''):
        super().__init__(dirpath=dirpath, filename=filename, seperator=seperator)
    ...
    def dump_data(self, new_data: Data) -> bool:
    ...
    def load_data(self, RecordInstance: AbstractRecord) -> Data:
        ds_data = Data()
        if create_connection := self.create_connection():
            txt_file = self.connection.Object(self.bucket_name, str(self.save_filepath)).get()['Body'].read().decode(
                'utf-8').splitlines()
        for row in txt_file:
            row = row.split(self.seperator)
            record = RecordInstance(key=row[0], value=row[1])
